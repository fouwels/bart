...
function cleanupCommands() {
	echo
	echo "$(date +%F-%X) - $BART_LOG_TAG Running cleanup commands"
	set -x
	if [ ${BACKUP_LDAP_ENABLED} == 'true' ]; then
		$DUPLICITYBIN cleanup ${GLOBAL_DUPLICITY_CACHE_PARMS} $DEST/ldap 2>&1
	fi
	if [ ${BACKUP_DB_ENABLED} == 'true' ]; then
		$DUPLICITYBIN cleanup ${GLOBAL_DUPLICITY_CACHE_PARMS} $DEST/db 2>&1
	fi
	if [ ${BACKUP_CONTENTSTORE_ENABLED} == 'true' ]; then
		$DUPLICITYBIN cleanup ${GLOBAL_DUPLICITY_CACHE_PARMS} $DEST/cs 2>&1
	fi
	set +x
}

# Main options
PARAMS="$PARAMS --allow-source-mismatch" #KF override as docker changes host ID
case $1 in
"backup")
	case $2 in
	"db")
